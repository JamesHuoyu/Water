...
# plt.legend()
# plt.xlabel(r"$\zeta$ (nm)")
# plt.title("KDE with different bandwidths")
# plt.savefig("/home/debian/water/TIP4P/2005/Tanaka_2018/rst/kde_bandwidth_comparison.png", dpi=300)
# # plt.show()
...
# plt.savefig("/home/debian/water/TIP4P/2005/Tanaka_2018/rst/histogram_bin_sensitivity.png", dpi=300)
# # plt.show()
...
print(
    f"2.5e-4:Fitted parameters:\nw1={w1:.5f}, mu1={mu1:.5f}, sigma1={sigma1:.5f}\nmu2={mu2:.5f}, sigma2={sigma2:.5f}"
)
# plot fit
plt.figure(figsize=(8, 5))
# plt.hist(zs, bins=100, density=True, alpha=0.3, label="Data histogram")
x_fit = np.linspace(zs.min(), zs.max(), 1000)
plt.plot(x_fit, two_gaussians(x_fit, *params), "-k", lw=2, label="Two-Gaussian Fit (2.5e-4)")
...
# w1, mu1, sigma1, mu2, sigma2 = params
# print(
#     f"2.5e-5:Fitted parameters:\nw1={w1:.5f}, mu1={mu1:.5f}, sigma1={sigma1:.5f}\nmu2={mu2:.5f}, sigma2={sigma2:.5f}"
# )
# # plot fit
# # plt.hist(zs, bins=100, density=True, alpha=0.3, label="Data histogram")
# x_fit = np.linspace(zs.min(), zs.max(), 1000)
# plt.plot(x_fit, two_gaussians(x_fit, *params), "-g", lw=2, label="Two-Gaussian Fit (2.5e-5)")
# # individual gaussians
# plt.plot(x_fit, w1 * norm.pdf(x_fit, mu1, sigma1), "--", label=f"Comp 1 (mu={mu1:.3f})")
# plt.plot(x_fit, (1 - w1) * norm.pdf(x_fit, mu2, sigma2), "--", label=f"Comp 2 (mu={mu2:.3f})")
...
# # plt.hist(zs, bins=100, density=True, alpha=0.3, label="Data histogram")
# x_fit = np.linspace(zs.min(), zs.max(), 1000)
# plt.plot(x_fit, two_gaussians(x_fit, *params), "-y", lw=2, label="Two-Gaussian Fit")
# # individual gaussians
# plt.plot(x_fit, w1 * norm.pdf(x_fit, mu1, sigma1), "--", label=f"Comp 1 (mu={mu1:.3f})")
# plt.plot(x_fit, (1 - w1) * norm.pdf(x_fit, mu2, sigma2), "--", label=f"Comp 2 (mu={mu2:.3f})")

# # plt.axvline(0, color="k", ls="--")
# # plt.xticks(np.arange(-0.10, 0.30, 0.05))
# # plt.yticks(np.arange(0, 12, 2))
plt.legend()
plt.xlabel(r"$\zeta$ (nm)")
plt.title("Two-Gaussian Fit to Histogram")
plt.savefig("/home/debian/water/TIP4P/2005/2020/rst/4096/2.5e-4/two_gaussian.png", dpi=300)
# plt.show()
