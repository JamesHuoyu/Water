...
def calculate_chi4_optimized(trajectory_file, a=1.0, stride=1, max_frames=None, n_processes=None):
    """
    ...
    for chunk_start in tqdm(range(0, n_frames, chunk_size), desc="Processing chunks"):
        ...
        Q_t += chunk_Q_t  # shape (n_frames,)
        Q_t2 += chunk_Q_t2
        count += chunk_count

    # Average and normalize
    valid_mask = count > 0  # Avoid division by zero
    Q_t[valid_mask] /= count[valid_mask]
    Q_t2[valid_mask] /= count[valid_mask]
    print("Finished processing all chunks.")
    print(f"Valid time points: {np.sum(valid_mask)} out of {n_frames}")
    print(f"Max count: {np.max(count)}, Min count (non-zero): {np.min(count[valid_mask])}")
    print(f"Count array (first 10): {count[:10]}")
    # Calculate chi4
    chi4_values = np.zeros_like(Q_t)
    chi4_values[valid_mask] = (Q_t2[valid_mask] - Q_t[valid_mask] ** 2) * (n_water)
...
def _process_chunk(positions, boxes, a, n_water, chunk_start, total_frames):
    ...
    for i in range(chunk_size):
        ...
        r0 = positions[i]  # shape (n_water, 3)
        box0 = boxes[i]
...
def _calculate_overlap_fast(r0, rt, box, a):
    ...
    diag_dist = np.diag(distances)

    # Count overlaps (distances <= a)
    overlaps = np.sum(diag_dist <= a) / len(diag_dist)
    return overlaps


def calculate_chi4_memory_efficient(
    trajectory_file, a=1.0, stride=1, max_tau=None, start_frame=0, end_frame=None
):
    """
    ...
    frame_indices = np.arange(start_frame, end_frame, stride)
    print(f"Total frames to process after applying stride: {len(frame_indices)}")
    print(f"Frame indices: {frame_indices[:10]} ... {frame_indices[-10:]}")
    n_frames = len(frame_indices)
    if max_tau is None:
        max_tau = n_frames
    else:
        max_tau = min(max_tau, n_frames)
    print(f"Processing {n_frames} frames with max_tau={max_tau}")

    # total_frames = len(u.trajectory) - start_frame
    # if max_tau is None:
    #     max_tau = total_frames
    # else:
    #     max_tau = min(max_tau, total_frames)

    # frame_indices = np.arange(start_frame, total_frames, stride)
    # n_frames = len(frame_indices)
    ...
    for frame_idx, global_frame in enumerate(tqdm(frame_indices, desc="Processing frames")):
        ...
        for i, (past_pos, past_box) in enumerate(zip(position_buffer[:-1], box_buffer[:-1])):
            past_frame_index = frame_indices[frame_idx - len(position_buffer) + 1 + i]
            tau = global_frame - past_frame_index  # 实际帧差
            if tau < max_tau:
...
if __name__ == "__main__":
    trajectory_file = "/home/debian/water/TIP5P/Tanaka_2018/rst/tip5p_246.lammpstrj"
    ...
    np.savetxt(
        "/home/debian/water/TIP5P/Tanaka_2018/rst/analyze/chi4_values_short.csv",
        results,
        delimiter=",",
        header="t,chi4",
        comments="",
    ...
    plt.xscale("log")
    plt.xlabel("Time (ps)")
    plt.xticks([0.01, 0.1, 1, 10, 100, 1000, 10000], ["-2", "-1", "0", "1", "2", "3", "4"])
    plt.ylabel(r"$\chi_4(t)$")
    plt.title("Four-point Susceptibility (Optimized)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(
        "/home/debian/water/TIP5P/Tanaka_2018/rst/analyze/chi4_values_optimized_short.png",
        dpi=300,
        bbox_inches="tight",
    )
    # plt.show()
