...
def load_edges(edges_csv: str | Path) -> pd.DataFrame:
    ...
    df = df.copy()
    df["O_idx_i"] = df[["O_idx_i", "O_idx_j"]].min(axis=1)
    df["O_idx_j"] = df[["O_idx_i", "O_idx_j"]].max(axis=1)
    return df.sort_values(["frame", "O_idx_i", "O_idx_j"]).reset_index(drop=True)


def analyze_counts_distribution(counts_df: pd.DataFrame, out_dir: Path, old_counts_csv: str | None = None) -> dict:
    out: dict = {}
    frame_summary = counts_df.groupby("frame")["n_hb"].agg(
        mean_n_hb="mean",
        frac4=lambda x: np.mean(x == 4),
        frac3=lambda x: np.mean(x == 3),
        frac5=lambda x: np.mean(x == 5),
    ).reset_index()
...
def _continuous_run_lengths(arr: np.ndarray) -> np.ndarray:
...
def _continuous_survival_from_run_lengths(lengths: Sequence[int], max_lag: int) -> np.ndarray:
...
def _continuous_correlation_exact(presence: Dict[Tuple[int, int], np.ndarray], max_lag: int) -> np.ndarray:
    """Exact continuous HB survival S_HB(t).

    Definition:
        S(t) = < h(0) H(t) > / < h(0) >
    where H(t)=1 only if the bond remained continuously present throughout [0, t].
    This guarantees S(0)=1 and S(t)>=0.
    """
    num = np.zeros(max_lag + 1, dtype=float)
    den = np.zeros(max_lag + 1, dtype=float)
    for arr in presence.values():
        n = len(arr)
        for lag in range(max_lag + 1):
            if lag >= n:
                break
            # start times where h(t0)=1
            a0 = arr[: n - lag]
            starts = np.where(a0 == 1)[0]
            if starts.size == 0:
                continue
            den[lag] += starts.size
            if lag == 0:
...
def _intermittent_correlation_exact(presence: Dict[Tuple[int, int], np.ndarray], max_lag: int) -> np.ndarray:
    """Standard intermittent HB correlation C_HB(t).

    Definition:
        C(t) = < h(0) h(t) > / < h(0) >
    computed over all observed pair time series, with a finite-window denominator.
    ...
    for arr in presence.values():
        ...
        for lag in range(max_lag + 1):
            ...
            h0 = arr[: n - lag]
            ht = arr[lag:]
            num[lag] += float(np.dot(h0, ht))
            den[lag] += float(np.sum(h0))
    out = np.zeros(max_lag + 1, dtype=float)
    valid = den > 0
    out[valid] = num[valid] / den[valid]
    out[0] = 1.0
...
def continuous_and_intermittent_lifetimes(edges_df: pd.DataFrame, out_dir: Path, max_lag: Optional[int] = None) -> dict:
    ...
    for arr in presence.values():
        run_lengths.extend(_continuous_run_lengths(arr).tolist())
    run_lengths_arr = np.asarray(run_lengths, dtype=np.int64)

    # We save both the exact continuous function and the run-length estimate.
    continuous_exact = _continuous_correlation_exact(presence, max_lag=max_lag)
    continuous_from_runs = _continuous_survival_from_run_lengths(run_lengths_arr, max_lag=max_lag)
    intermittent = _intermittent_correlation_exact(presence, max_lag=max_lag)

    pd.DataFrame({
        "lag": np.arange(len(intermittent)),
        "continuous_survival_exact": continuous_exact,
        "continuous_survival_from_runs": continuous_from_runs,
        "intermittent_correlation": intermittent,
    }).to_csv(out_dir / "hb_lifetime_correlations.csv", index=False)
    pd.DataFrame({"continuous_run_length_frames": run_lengths_arr}).to_csv(out_dir / "hb_continuous_run_lengths.csv", index=False)

    plt.figure(figsize=(7, 5))
    plt.plot(np.arange(len(continuous_exact)), continuous_exact, label="S_HB(t) exact")
    ...
    return {
        ...
        "intermittent_correlation_lag0": float(intermittent[0]) if intermittent.size else None,
        "continuous_exact_lag0": float(continuous_exact[0]) if continuous_exact.size else None,
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Postprocess HOO HB network for analyses 1-4 (fixed intermittent correlation)")
