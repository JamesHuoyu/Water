...
def main() -> None:
    ...
    p.add_argument("--zeta_csv", required=True, help="zeta.csv or zeta_valid.csv from the HOO zeta script")
    p.add_argument("--zeta_cg_csv", default=None, help="Existing zeta_cg.csv. If omitted, it will be generated.")
    ...
    py = sys.executable
    run([
        py, "/mnt/data/hb_extract_hoo_timeseries.py",
        "--dump_file", args.dump_file,
        "--out_dir", str(hb_dir),
        "--oo_cutoff", str(args.oo_cutoff),
        "--hoo_cutoff", str(args.hoo_cutoff),
        "--oh_cutoff", str(args.oh_cutoff),
    ])

    cmd = [py, "/mnt/data/hb_dynamics_postprocess.py", "--hb_dir", str(hb_dir), "--out_dir", str(hb_post)]
    ...
    if zeta_cg_csv is None:
        ...
        run([
            py, "/mnt/data/zeta_cg_hoo_updated.py",
            "--dump_file", args.dump_file,
            "--zeta_file", args.zeta_csv,
            "--output_csv", zeta_cg_csv,
            "--cutoff", str(args.cg_cutoff),
            "--kernel_length", str(args.kernel_length),
        ])

    cmd = [
        py, "/mnt/data/zeta_dynamics_postprocess.py",
        "--zeta_csv", args.zeta_csv,
        "--zeta_cg_csv", zeta_cg_csv,
        "--out_dir", str(zeta_dir),
    ]
    if args.max_lag is not None:
        cmd.extend(["--max_lag", str(args.max_lag)])
    run(cmd)
